...
namespace Skybound.Gecko
{
  ...
  //nsIWindowProvider,
  {
    #region scroller const
    const int ScrollOrientation_X = 1;
    const int ScrollOrientation_Y = 2;
    const int Scrollbar_Auto = 1;
    const int Scrollbar_Never = 2;
    const int Scrollbar_Always = 3;
    #endregion
    ...
    protected override void OnHandleCreated(EventArgs e)
    {
      if (!this.DesignMode)
      {
        ...
        Scroller = Xpcom.QueryInterface<nsIScrollable>(WebBrowser);
    ...
    /// <summary>
    /// interface to the zoom level of the browser
    /// </summary>
    /// <param name="zoomStep"></param>
    private void SetZoom(float zoomStep)
    {
      nsIWebNavigation nav = Xpcom.QueryInterface<nsIWebNavigation>(WebBrowser);
      nsIDocShell shell = Xpcom.QueryInterface<nsIDocShell>(nav);
      nsIContentViewer v;
      shell.getContentViewer(out v);
      nsIMarkupDocumentViewer view = Xpcom.QueryInterface<nsIMarkupDocumentViewer>(v);
      view.setFullZoom(zoomStep);
    }

    /// <summary>
    /// set/get the zoom level for the browser
    /// </summary>
